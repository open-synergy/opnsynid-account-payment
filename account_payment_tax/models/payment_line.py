...
class PaymentLine(models.Model):
    ...
    @api.multi
    @api.depends("tax_ids", "amount_currency", "partner_id", "currency")
            ...
            if rec.tax_ids:
                ...
                for tax in rec.tax_ids:
                    taxes = tax.compute_all(
                        price_unit=price, quantity=1.0, partner=rec.partner_id
                    )
                    taxes_total = taxes["total"]
                    for c in taxes["taxes"]:
                        total += c.get("amount", 0.0)
                amount_total = taxes_total + total
            else:
                amount_total = rec.amount_currency
            curr = rec.currency
            rec.amount_tax_currency = curr.round(total)
    ...
    amount_tax_currency = fields.Float(
        string="Amount Tax", store=False, compute="_compute_amount"
    )

    amount_total_currency = fields.Float(
        string="Amount Total", store=False, compute="_compute_amount"
    )
